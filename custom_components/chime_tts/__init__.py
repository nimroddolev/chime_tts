...
def get_tts_segment_cache_hash(segment_params, segment_options, resolved_platform, used_fallback):
    """Return the cache key for a generated TTS segment."""
    cache_params = dict(segment_params)
    if used_fallback and resolved_platform:
        cache_params["tts_platform"] = resolved_platform
    return get_filename_hash_from_service_data({**cache_params}, {**segment_options})
                    ...
                    if tts_audio is not None:
                        ...
                        if (
                            ...
                            if tts_audio_full_path is not None:
                                ...
                                segment_filepath_hash = get_tts_segment_cache_hash(
                                    segment_params,
                                    segment_options,
                                    tts_audio_helper.last_tts_platform,
                                    tts_audio_helper.last_request_used_fallback,
                                )
