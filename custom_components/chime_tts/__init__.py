...
async def async_update_configuration(config_entry: ConfigEntry, hass: HomeAssistant = None):
    ...
    # Cache audio produced by the fallback TTS platform (off by default: a clip
    # generated by the fallback platform during an outage would otherwise keep
    # being served from the cache after the primary platform recovers, because
    # the cache key is built from the *requested* platform)
...
async def async_get_playback_audio_path(params: dict, options: dict):
    ...
    output_audio = None
    tts_audio_helper.reset_fallback_tracking()
    ...
    # Save path to cache. Audio that came from the fallback TTS platform is
    # skipped unless caching fallback audio is enabled: the cache key is built
    # from the requested platform, so caching it would keep serving the
    # fallback's voice after the primary platform recovers.
