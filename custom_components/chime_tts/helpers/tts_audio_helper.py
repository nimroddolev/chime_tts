...
class TTSAudioHelper:
    ...
    # Whether the most recent top-level request fell back to the fallback
    # platform, and whether any request in the current service call did.
    ...
    def may_cache_request_audio(self) -> bool:
        ...
        return self.cache_fallback_audio or not self._used_fallback
    ...
    async def _retry_with_fallback(self, hass: HomeAssistant, tts_platform, message, language, cache, options):
        ...
        if tts_platform != fallback_platform and fallback_platform:
            ...
            # Keep the fallback platform's clip out of Home Assistant's own TTS
            # cache too, unless caching fallback audio is explicitly enabled.
            fallback_cache = cache and self.cache_fallback_audio
            if cache and not fallback_cache:
                _LOGGER.debug(
                    "Audio generated by the fallback platform will not be cached"
                )
