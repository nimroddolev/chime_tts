...
class MediaPlayerHelper:
    ...
    async def async_initialize_media_players(
        ...
        for entity_id in entity_ids:
            media_player_object = await self.async_get_media_player_object(
                hass, entity_id, volume_level
            )
    ...
    async def async_get_media_player_object(
        self, hass: HomeAssistant, entity_id: str, target_volume_level
    ):
    ...
    def get_fade_in_out_media_players(self) -> list[ChimeTTSMediaPlayer]:
        ...
        for media_player in self.media_players:
            if media_player.initially_playing and (
                self.fade_audio
                or (self.announce and not media_player.announce_supported)
            ):
    ...
    def get_set_volume_media_players(self) -> list[ChimeTTSMediaPlayer]:
        ...
        for media_player in self.media_players:
            if (
                media_player not in self.get_fade_in_out_media_players()
                and media_player.target_volume_level
                not in [-1, media_player.initial_volume_level]
    ...
    def get_media_players_from_entity_ids(
        self, entity_ids
    ...
    def get_is_standard_media_player(self, entity_id):
        ...
        return platform and platform not in (
            ALEXA_MEDIA_PLAYER_PLATFORM,
            SONOS_PLATFORM,
            SPOTIFY_PLATFORM,
        )
    ...
    def get_platform_from_entity_id(self, entity_id):
        ...
        media_player: ChimeTTSMediaPlayer = self.get_media_players_from_entity_id(
            entity_id
        )
        if media_player:
    ...
    def get_is_media_player_alexa(self, entity_id):
        ...
        return (
            self.get_platform_from_entity_id(entity_id) == ALEXA_MEDIA_PLAYER_PLATFORM
        )
    ...
    def get_alexa_media_players_count(self):
        ...
        alexa_media_players = [
            media_player
            for media_player in self.media_players
            if media_player.platform == ALEXA_MEDIA_PLAYER_PLATFORM
        ]
    ...
    def get_media_players_of_platform(self, entity_ids: list = [], platform: str = ""):
        ...
        if entity_ids and platform:
            return [
                entity_id
                for entity_id in entity_ids
                if self.get_media_players_from_entity_id(entity_id)
                and self.get_media_players_from_entity_id(entity_id).platform
                == platform
            ]
        return []
    ...
    async def async_fade_out_and_pause(self, hass: HomeAssistant, fade_duration: float):
        ...
        fade_in_out_media_players: list[ChimeTTSMediaPlayer] = (
            self.get_fade_in_out_media_players()
        )
        if len(fade_in_out_media_players) > 0:
            ...
            try:
                ...
                _LOGGER.warning(
                    "Unable to pause media player%s: %s",
                    ("" if len(pause_entity_ids) == 1 else "s"),
                    str(error),
                )
    ...
    async def async_resume_playback(self, hass, fade_duration: float):
        ...
        if len(fade_in_media_players) > 0:
            ...
            if not await self.async_wait_until_media_players_state_is(
                hass=hass,
                ...
                _LOGGER.warning(
                    "Timed out waiting for %s media_player%s to pause",
            ...
            try:
                ...
                _LOGGER.warning(
                    "Unable to set %s's volume to 0 for: %s. Error: %s",
                    entity_id,
                    (", ".join(map(str, resume_entity_ids))),
                    error,
                )
            ...
            while len(paused_media_players) > 0 and retry_duration > 0:
                ...
                for entity_id in paused_media_players:
                    ...
                    if entity_state is None or entity_state.state != "playing":
    ...
    async def async_wait_until_media_players_state_is(
        ...
        _LOGGER.debug(
            " - Waiting until %s media_player%s %s %s...",
            len(media_players),
            ("" if len(media_players) == 1 else "s"),
            ("is" if len(media_players) == 1 else "are"),
            target_state,
        )
        return await self._async_wait_until_media_players(
            hass, media_players, condition, timeout
        )
    ...
    async def async_wait_until_media_players_volume_level_is(
        self,
        hass: HomeAssistant,
        media_players: list[ChimeTTSMediaPlayer],
        target_volume: str,
        ...
        _LOGGER.debug(
            " - Waiting until %s media_player%s volume_level %s %s...",
            len(media_players),
            ("" if len(media_players) == 1 else "s"),
            ("is" if len(media_players) == 1 else "are"),
            target_volume,
    ...
    async def _async_wait_until_media_players(
        self,
        hass: HomeAssistant,
        media_players: list[ChimeTTSMediaPlayer],
        condition,
        ...
        if (
            hass is None
            or media_players is None
            or len(media_players) == 0
            ...
            for media_player in media_players:
                if condition(media_player) and media_player in still_waiting:
                    ...
                    try:
                        ...
                        _LOGGER.error(
                            "Error updating media player %s's state: %s",
        ...
        if len(still_waiting) > 0:
            for media_player in still_waiting:
                _LOGGER.debug(
                    "   𝘅 %s - Timed out. Current state: %s",
                    media_player.entity_id,
                    str(media_player.get_state()),
                )
    ...
    async def async_sonos_snapshot(self, hass: HomeAssistant):
        ...
        if not SONOS_SNAPSHOT_ENABLED:
            return
        sonos_media_player_entity_ids: list[str] = [
            media_player.entity_id
            for media_player in self.media_players
            if media_player.platform == SONOS_PLATFORM
        ]
        if len(sonos_media_player_entity_ids) > 0:
            _LOGGER.debug(
                "Taking a Sonos snapshot of %s media player%s",
                str(len(sonos_media_player_entity_ids)),
                "" if len(sonos_media_player_entity_ids) == 1 else "s",
            )
            try:
                await hass.services.async_call(
                    domain="sonos",
    ...
    async def async_sonos_restore(self, hass: HomeAssistant):
        ...
        if len(sonos_media_player_entity_ids) > 0:
            _LOGGER.debug(
                "Restoring %s Sonos media player%s from snapshot",
                str(len(sonos_media_player_entity_ids)),
                "" if len(sonos_media_player_entity_ids) == 1 else "s",
            )
            try:
    ...
    async def async_set_volume_for_media_players(
        self,
        hass: HomeAssistant,
        media_players: list[ChimeTTSMediaPlayer],
        volume_key,
        ...
        for media_player in media_players:
            ...
            target_volume: float = (
                getattr(media_player, volume_key, 0)
                if isinstance(volume_key, str)
                else volume_key
            )
            ...
            if target_volume == -1:
                if volume_key == "initial_volume_level":
                    _LOGGER.debug(
                        "Initial volume for %s is unknown. Unable to restore volume.",
                        entity_id,
                    )
                continue
            ...
            if target_volume == current_volume:
                _LOGGER.debug(
                    "The volume level for %s is already set to %s",
                    entity_id,
                    str(target_volume),
                )
                continue

            volume_changed_dicts.append(
                {"media_player": media_player, "target_volume": target_volume}
            )

            if fade_steps > 1:
                volume_step: float = (target_volume - current_volume) / fade_steps
                volume_steps: list[float] = [
                    current_volume + volume_step * i for i in range(1, fade_steps + 1)
                ]
                _LOGGER.debug(
                    " - Fading %s %s's volume from %s to %s over %ss",
                ...
                if target_volume > current_volume:
                    _LOGGER.debug(
                        "Increasing %s's volume from %s to %s",
                        entity_id,
                        str(current_volume),
                        str(target_volume),
                    )
                    ...
                    _LOGGER.debug(
                        "Decreasing %s's volume from %s to %s",
                        entity_id,
                        str(current_volume),
                        str(target_volume),
                    )
    ...
    async def async_set_volume_action(
        self,
        hass: HomeAssistant,
        entity_id: str,
        target_volume: float,
        ...
        if is_retry:
            _LOGGER.debug(
                " - Retry setting %s's volume to %s", entity_id, str(target_volume)
            )
        else:
        ...
        try:
            ...
            _LOGGER.warning(
                "Unable to set %s's volume to %s: %s",
                entity_id,
                str(target_volume),
                error,
            )
    ...
    async def async_wait_until_target_volume_reached(self, hass, volume_changed_dicts):
        ...
        while time.time() < end_time:
            ...
            for media_player_dict in volume_changed_dicts[
                ...
                media_player: ChimeTTSMediaPlayer = media_player_dict.get(
                    "media_player"
                )
                target_volume = media_player_dict.get("target_volume")

                if media_player and round(
                    media_player.get_current_volume_level(), 3
                ) not in (round(target_volume, 3), round(-1, 3)):
                    ...
                    await self.async_set_volume_action(
                        hass, media_player.entity_id, target_volume, True
                    )
                else:
                    _LOGGER.debug(
                        " - ✔️ %s's volume now %s",
                        media_player.entity_id,
                        str(media_player.get_current_volume_level()),
                    )
                    volume_changed_dicts.remove(
                        media_player_dict
                    )  # Remove the entry once the volume is set
        ...
        for media_player_dict in volume_changed_dicts:
            ...
            if media_player:
                ...
                if current_volume != target_volume:
                    _LOGGER.debug(
                        " - 𝘅 %s (timed out before volume set to %s. Current volume = %s)",
                        media_player.entity_id,
                        str(target_volume),
                        str(current_volume),
                    )
    ...
    async def async_join_media_players(self, hass: HomeAssistant):
        ...
        if joined_count == 0:
            _LOGGER.warning(
                "No media_players were found that support joining speakers into a group. A minimum of 2 is required."
            )
            return
        if joined_count == 1:
            _LOGGER.warning(
                "Only 1 media_player was found that supports joining speakers into a group. A minimum of 2 is required."
            )
            return
    ...
    async def async_unjoin_media_players(self, hass):
        ...
        if self.unjoin_players is True and self.joined_entity_id:
            ...
            media_player_entity_ids: list[str] = self.joined_media_player_entity_ids + [
                self.joined_entity_id
            ]
            count = 0
            for entity_id in media_player_entity_ids:
                count += 1
                _LOGGER.debug(
                    "     - media_player.unjoin %s/%s: %s",
                    str(count),
                    str(len(media_player_entity_ids)),
                    entity_id,
                )
                try:
                    await hass.services.async_call(
