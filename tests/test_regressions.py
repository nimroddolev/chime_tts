...
def _run_playback_path(monkeypatch, *, used_fallback, cache_fallback):
    ...
    async def fake_process_segments(hass, message, output_audio, params, options):
        """Stand in for TTS generation, optionally having used the fallback."""
        if used_fallback:
            helper._used_fallback = True
    ...
    monkeypatch.setattr(
        integration_module.filesystem_helper,
        "async_get_chime_path_with_offset",
        AsyncMock(side_effect=[(None, None), (None, None)]),
    )
    monkeypatch.setattr(
        integration_module.media_player_helper, "get_alexa_media_players_count", lambda: 0
    )
    monkeypatch.setattr(
        integration_module.media_player_helper,
        "get_media_content_id",
        lambda *args, **kwargs: "media-content-id",
    )
    monkeypatch.setattr(integration_module, "async_verify_cached_audio", AsyncMock(return_value=None))
    monkeypatch.setattr(integration_module, "async_get_audio_from_path", AsyncMock(return_value=segment))
    monkeypatch.setattr(integration_module, "async_process_segments", fake_process_segments)
    monkeypatch.setattr(
        integration_module.filesystem_helper,
        "async_save_audio_to_folder",
        AsyncMock(return_value="/tmp/chime_tts_regression.mp3"),
    )
    monkeypatch.setattr(
        integration_module.filesystem_helper, "async_load_audio", AsyncMock(return_value=segment)
    )
    monkeypatch.setattr(
        integration_module,
        "async_save_audio_to_folder",
        AsyncMock(side_effect=lambda hass, is_local, is_public, audio_dict, output_audio: audio_dict),
    )
    monkeypatch.setattr(integration_module, "validate_audio_dict", lambda *args, **kwargs: True)
    ...
    class _Hass:
        ...
        config = SimpleNamespace(path=lambda *parts: "/config", media_dirs={})
    ...
    asyncio.run(integration_module.async_get_playback_audio_path(params, {}))
    return add_to_cache


def test_issue_303_fallback_audio_is_not_written_to_the_cache(monkeypatch):
    """Audio generated by the fallback TTS platform must not be cached by default."""
    add_to_cache = _run_playback_path(monkeypatch, used_fallback=True, cache_fallback=False)
    add_to_cache.assert_not_awaited()
...
def test_issue_303_primary_audio_is_still_cached(monkeypatch):
    ...
    add_to_cache = _run_playback_path(monkeypatch, used_fallback=False, cache_fallback=False)
    assert add_to_cache.await_count > 0


def test_issue_303_fallback_audio_is_cached_when_opted_in(monkeypatch):
    """Enabling the fallback cache option restores the previous caching behaviour."""
    add_to_cache = _run_playback_path(monkeypatch, used_fallback=True, cache_fallback=True)
    assert add_to_cache.await_count > 0
