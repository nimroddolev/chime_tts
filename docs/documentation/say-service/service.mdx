...
Each TTS platform offers its own set of options, which can also be used with Chime TTS in the `options` parameter. Several key TTS platform parameters exist as separate parameters in Chime TTS (see `chime_tts.say`'s [parameters](../say-service/parameters) documentation for more).
...
- **Between** TTS audio segments *(see the use of* [`message`](../say-service/parameters#advanced-usage-segment-sequences) *segments).*
