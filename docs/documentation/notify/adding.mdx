...
# Adding Chime TTS Notifications

1. Open your Home Assistant `configuration.yaml` file.
2. Locate or add the `notify` section within your configuration.
3. Add 1 or more notification entires (see the example below).
4. Include any [`chime_tts.say`](../say-service) [parameters](../say-service/parameters) and set the default values.

### Basic Chime TTS notify entry
_configuration.yaml_
```
notify:
  - name: announcements  # (required) Choose a recognizable name for your notifications
