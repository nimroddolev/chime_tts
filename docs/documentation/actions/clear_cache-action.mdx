...
The `chime_tts.clear_cache` action can be used to remove cached audio files from the [`chime_tts.say`](./say-action) and [`chime_tts.say_url`](./say_url-action) actions as well as Home Assistant's TTS cache.
