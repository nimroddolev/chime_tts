...
`chime_path` and `end_chime_path` accept a [Chime Set](../../chime-sets) name as well as an individual chime. The selector shows sets with a 🎲 prefix. At runtime Chime TTS randomly chooses an available member; nested sets are not supported.
...
This field is available only for `chime_tts.say`; it is not available for `chime_tts.say_url`.
