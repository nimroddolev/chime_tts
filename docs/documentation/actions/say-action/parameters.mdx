...
The `chime_tts.say` action has [standard parameters](#standard-parameters) used by all TTS platforms, and [additional parameters](#additional-parameters) used by specific TTS platforms.
...
`message` uses Home Assistant's template selector, so templates can be used wherever Home Assistant allows templates:

```yaml
message: "The temperature is {{ states('sensor.kitchen_temperature') }} degrees."
```
...
<sup>*</sup>To use a TTS platform not included in the list, simply set the custom TTS platform's action as the `tts_platform` parameter value.
...
| -------------------- | -------------------------------------------------------------- |
| **Required:**        | No                                                             |
| **Example 1:**       | One of the Chime TTS presets:<br/>`airport`<br/>`ba_dum_tss`<br/>`bells`<br/>`bells_2`<br/>`bright`<br/>`chirp`<br/>`choir`<br/>`chord`<br/>`classical`<br/>`crickets`<br/>`ding_dong`<br/>`doorbell`<br/>`drumroll`<br/>`dun_dun_dun`<br/>`error`<br/>`fanfare`<br/>`glockenspiel`<br/>`hail`<br/>`knock`<br/>`marimba`<br/>`mario_coin`<br/>`microphone_tap`<br/>`sad_trombone`<br/>`soft`<br/>`tada`<br/>`toast`<br/>`twenty_four`<br/>`whistle` |
| **Example 2:**       | One of your own custom chimes : `My Custom Chime`<br/>(_see [Custom Chimes Folder](../../chimes#custom-chimes-folder)_) |
| **Example 3:**       | A [Chime Set](../../chime-sets) name: `🎲 Front Door Chimes` |
| **Example 4:**       | A local file:<br/>`/media/sounds/notification.mp3` |
| **Example 5:**       | A file from a remote URL:<br/>`https://www.website.com/sounds/notification.ogg` |
| **Default:**         | None                                                           |
...
| **Description:** | The duration of time (in milliseconds) to gradually decrease the volume of an audio segment while simultaneously increasing the volume of the next. |
| ---------------- | ---------------------------------------------------------------------------------------------- |
| **Required:**    | No                                                                                             |
| **Example:**   | `500` (Crossfades segments over a half second)                                                 |
| **Default:**     | `0`                                                                                          |
...
`initial_delay` is a beta parameter and may change between releases.

:::

| **Description:** | The silence (in milliseconds) to insert before the complete generated audio, including any start chime. |
| ---------------- | -------------------------------------------------------------------------------------------------------- |
| **Required:**    | No                                                                                                       |
| **Example:**     | `200` (Adds 200 milliseconds of silence before the announcement)                                        |
| **Default:**     | `0`                                                                                                      |

When omitted, the configured [default initial delay](/chime_tts/docs/documentation/configuration/playback-options#default-initial-delay-beta) is used. A value supplied in the action takes precedence.
...
The `pre_script` is a beta parameter and may change between releases.

:::

Run a script immediately before playback. This action value overrides the configured default `pre_script`.
...
This field is available for both `chime_tts.say` and `chime_tts.say_url`. For `chime_tts.say`, it runs immediately before playback; for `chime_tts.say_url`, it runs before audio generation.
...
The `post_script` is a beta parameter and may change between releases.

:::

Run a script after `chime_tts.say` playback or `chime_tts.say_url` audio generation. This action value overrides the configured default `post_script`.
...
This field is available for both `chime_tts.say` and `chime_tts.say_url`. For `chime_tts.say`, it runs after playback; for `chime_tts.say_url`, it runs after audio generation.
...
```yaml
post_script:
  script: script.front_door_restore
  data:
    volume: 0.5
```
...
`repeat` is a beta parameter and may change between releases.

:::

Repeat the complete assembled announcement-start chime, message, and end chime-after its initial playback. `repeat` is the number of **additional** plays: use `0` for one total playback (no repeat), `1` for two total plays, and so on.

| **Required:** | No |
| --- | --- |
| **Example:** | `2` (three total plays) |
| **Default:** | `0` |
...
`repeat_delay` is a beta parameter and may change between releases.

:::

Set the silence, in milliseconds, inserted between each repeated copy of the complete announcement. This applies only when `repeat` is greater than `0`.

| **Required:** | No |
| --- | --- |
| **Example:** | `500` (half a second between copies) |
| **Default:** | `0` |
...
The cache key includes `repeat`, `repeat_delay`, and the current repeat semantics. Audio generated with earlier repeat behavior is therefore not reused for a repeated announcement.
