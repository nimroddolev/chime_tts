...
When omitted, the configured [default initial delay](/chime_tts/docs/documentation/configuration/playback-options#default-initial-delay) is used. A value supplied in the action takes precedence.
...
The `pre_script` and `post_script` controls are currently in beta and may change between releases.
...
The action-level `repeat` and `repeat_delay` controls are currently in beta and may change between releases.
