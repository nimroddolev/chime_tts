...
## Parameters
The [parameters](/chime_tts/docs/documentation/actions/say-action/parameters) page provides explanations for each parameter supported by Chime TTS
...
Each TTS platform offers its own set of options, which can also be used with Chime TTS in the `options` parameter. Several key TTS platform parameters exist as separate parameters in Chime TTS (*see* `chime_tts.say`*'s [additional parameters](/chime_tts/docs/documentation/actions/say-action/parameters#additional-parameters) documentation for more*).

## Chimes

Chime audio files can be incorporated into your notifications in a number of ways:

- **Before** TTS audio *(using the* [`chime_path`](/chime_tts/docs/documentation/actions/say-action/parameters#chime_path) *parameter).*
- **After** TTS audio *(using the* [`end_chime_path`](/chime_tts/docs/documentation/actions/say-action/parameters#end_chime_path) *parameter)*, and
- **Between** TTS audio segments *(using* [`message`](/chime_tts/docs/documentation/actions/say-action/parameters#advanced-usage-segment-sequences) *segments).*

### Selecting Chimes

You can specify which chime/s to incorporate into your notifications by selecting a chime from the [drop-down lists in the user interface](#selecting-chimes-from-the-user-interface) for both the [`chime_path`](/chime_tts/docs/documentation/actions/say-action/parameters#chime_path) and [`end_chime_path`](/chime_tts/docs/documentation/actions/say-action/parameters#end_chime_path) parameters, choosing a [Chime Set](/chime_tts/docs/documentation/chime-sets), or specifying [local](#local-files) and [remote](#external-audio-file-urls) audio-file paths.

Chimes and Chime Sets are managed from **Settings → Devices & services → Chime TTS → Configure**. A Chime Set randomly selects an available member when the action runs.
...
**After restarting Home Assistant**, the chimes will be added to the [`chime_path`](/chime_tts/docs/documentation/actions/say-action/parameters#chime_path) and [`end_chime_path`](/chime_tts/docs/documentation/actions/say-action/parameters#end_chime_path) drop down lists.
...
* #### External audio file URLs
  You can also specify URLs to audio files hosted online, which will be downloaded and optionally saved locally them to your [Downloaded Chimes Folder](../../configuration) when the [`cache`](/chime_tts/docs/documentation/actions/say-action/parameters#cache) property is set to `true`.
