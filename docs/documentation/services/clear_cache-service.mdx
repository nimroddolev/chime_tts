...
The `chime_tts.clear_cache` service can be used to remove cached audio files from the [`chime_tts.say`](./say-service) and [`chime_tts.say_url`](./say_url-service) services as well as Home Assistant's TTS cache.
