...
**Please note** that Chime TTS works __with__ [TTS Platform integrations](https://www.home-assistant.io/integrations/#text-to-speech), but you need to add them to your Home Assistant instance beforehand.
...
- **Before** TTS audio *(see the* [`chime_path`](../say-service/parameters#chime_path) *parameter).*
- **After** TTS audio *(see the* [`end_chime_path`](../say-service/parameters#end_chime_path) *parameter)*, and
- **Between** TTS audio *(see the use of* [`message`](../say-service/parameters#advanced-usage-segment-sequences) *segments).*

There are 3 different ways to define which chime/s to include in your notification:

### 1. Select chimes from the user-interface
...
* #### Local Files
  You can specify local file paths to your own audio files from your Home Assistant instance's media folder, or any other folder.
...
* #### External audio file URLs
  You can also specify URLs to audio files hosted online. Chime TTS will download the files and you can optionally save them to your chimes folder if the [`cache`](../say-service/parameters#cache) property is set to `true`.
